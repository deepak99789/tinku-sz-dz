...
def build_alert_text(tkr: str, itv: str, event: dict, rr_target: float) -> str:
    z = event["zone"]
    label = f"{tkr} [{itv}]"
...
# RUN SCAN
# ==========================================================================
st.session_state.setdefault("sent_alert_keys", set())
...
if trigger_scan:
    ...
    # ---- Collect NEW (non-duplicate) events across all combos ----
    new_alerts = []
    for (tkr, itv), data in combo_results.items():
        ...
        for e in events:
            ...
            if key in st.session_state["sent_alert_keys"]:
                continue  # already alerted before -> skip (no duplicate)
            st.session_state["sent_alert_keys"].add(key)
            txt = build_alert_text(tkr, itv, e, rr_target)
            new_alerts.append({
                "key": key, "ticker": tkr, "interval": itv, "type": e["type"],
                "text": txt, "time": dt.datetime.now().strftime("%H:%M:%S"),
            })
    ...
    if new_alerts:
        ...
        if telegram_on:
            for a in new_alerts:
                ok, msg = send_telegram_message(bot_token, chat_id, a["text"])
                if not ok:
                    st.warning(f"Telegram ({a['ticker']} [{a['interval']}]): {msg}")

        # ---- In-app log (newest first, capped) ----
        st.session_state["alert_log"] = (new_alerts[::-1] + st.session_state["alert_log"])[:100]

        # ---- In-app toast popups ----
        if toast_on:
            for a in new_alerts[:5]:  # avoid flooding if a big batch fires at once
                st.toast(f"{ALERT_ICONS.get(a['type'], '🔔')} {a['ticker']} [{a['interval']}] — {a['type'].replace('_', ' ').title()}", icon=ALERT_ICONS.get(a["type"], "🔔"))
...
if "combo_results" in st.session_state:
    ...
    with dc:
        ...
        def get_base_count(pattern_name: str) -> int:
            """Pattern name me tag se base-candle count nikalta hai:
            ' 3C' => 3 base candles, ' 2C' => 2 base candles,
            koi tag nahi (std/ext) => 1 base candle."""
            if "3C" in pattern_name:
                return 3
            if "2C" in pattern_name:
                return 2
            return 1
        ...
        for (tkr, itv), data in combo_results.items():
            ...
            for z in r.all_zones:
                ...
                rows.append({
                    ...
                    "Base Count": get_base_count(z.pattern_name),
