...
with settings_box:
    ...
    row2b_c1, row2b_c2 = st.columns([1.6, 3])
    with row2b_c1:
        validation_checklist_on = st.checkbox(
            "✅ Validation Checklist (Legin/Legout)", value=False,
            help=(
                "ON karne par RBD/DBD/DBR/RBR har pattern par 2 extra validation "
                "conditions lagengi:\n\n"
                "1) Legout candle ka open, legin candle ki High-Low range ke andar hi hona chahiye "
                "(legin se upar/niche gap na ho).\n\n"
                "2) Legout candle ka open, base candle ke body (open/close) se clean gap ke saath hona chahiye "
                "- base candle ki body legout ki body se cover nahi honi chahiye (sirf wick cover ho sakta hai).\n\n"
                "OFF rehne par default zone-finding condition hi chalegi (koi extra filter nahi)."
            ),
        )
    with row2b_c2:
        if validation_checklist_on:
            st.caption(
                "🔎 Validation checklist ON hai — sirf strict legin/legout gap wale zones hi dikhenge."
            )
...
if trigger_scan:
    ...
    for tkr in final_tickers:
        for itv in final_intervals:
            ...
            result = run_full_pipeline(
                ...
                validation_checklist=validation_checklist_on,
            )
    ...
    st.session_state["validation_checklist_on"] = validation_checklist_on
